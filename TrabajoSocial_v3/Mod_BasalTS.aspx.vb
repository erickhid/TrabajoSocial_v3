...
Partial Class Mod_BasalTS
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            Else
                ...
                'pnl_CircuitoAdherencia.Visible = False
                Dim dt As New DataTable
                dt.Columns.Add("IdGrupoFamiliar_Indice")
                ...
                'dt.Rows.Add(0, "", "", "", "", "", "", "")
                Session("datosGF_Mod") = dt
                'txt_ingpersonal.Text = Nothing
                'txt_inghogar.Text = Nothing
                'txt_egvivienda.Text = Nothing
                'txt_egeelectrica.Text = Nothing
                'txt_egagua.Text = Nothing
                'txt_egcable.Text = Nothing
                'txt_egtelefono.Text = Nothing
                'txt_egalimentacion.Text = Nothing
                'txt_egtransporte.Text = Nothing
                'txt_egeducacion.Text = Nothing
                'txt_egbasura.Text = Nothing
                'txt_egotros.Text = Nothing
    ...
    Sub llenaGrupoFamiliar(ByVal IdBasalesTS As String)
        Try
            db.Cn1 = cn1
            If IdBasalesTS <> "" Then
                Dim GrupoFamiliar As DataTable = db.ObtieneGrupoFamiliar(IdBasalesTS, usuario)

                If GrupoFamiliar.Rows.Count <> 0 Then
                    For Each row As DataRow In GrupoFamiliar.Rows
                        lbl_Grupo_Fam.Text = row("IdGrupoFamiliar").ToString()
                    Next
                End If

                Session("datosGF_Mod") = GrupoFamiliar
    ...
    Sub buscaNHC()
        If txt_asi.Text.ToUpper().Trim <> String.Empty Then
            ...
            'divingresoCircuito.Visible = True
            'If existenhc Then
            '    If ufecha Then
    ...
    Protected Sub ibt_Agregar_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs) Handles ibt_Agregar.Click
        ...
        Else
            ...
            Dim dt As DataTable = Session("datosGF_Mod")
                Dim cont As Integer = 0
                Dim IdGF As Integer = 0

                For Each row As DataRow In dt.Rows
                    cont = Convert.ToInt32(row("IdGrupoFamiliar_Indice").ToString())
                    IdGF = Convert.ToInt32(row("IdGrupoFamiliar").ToString())
                Next


            If txt_Nombre.Text.ToString() <> "" Then
                If txt_Edad.Text.ToString() <> "" Then
                    dt.Rows.Add(cont + 1, IdGF + 1, txt_Nombre.Text.ToString(), DDL_TipoRelacion.SelectedItem.Text.ToString(), txt_Edad.Text.ToString(), DDL_NivelEducativo.SelectedItem.Text.ToString(), DDL_SituacionLaboral.SelectedItem.Text.ToString(), txt_Ingreso.Text.ToString(), DDL_ConoceDx.SelectedItem.Text.ToString())
                    Session("datosGF_Mod") = dt
            ...
            'dt.Rows.Add(cont + 1, lbl_id_basal_ts.Text.ToString(), txt_Nombre.Text.ToString(), DDL_TipoRelacion.SelectedItem.Text.ToString(), txt_Edad.Text.ToString(), DDL_NivelEducativo.SelectedItem.Text.ToString(), DDL_SituacionLaboral.SelectedItem.Text.ToString(), txt_Ingreso.Text.ToString(), DDL_ConoceDx.SelectedItem.Text.ToString())
            'Session("datosGF_Mod") = dt
            'GV_GrupoFamiliar.DataSource = dt
            'GV_GrupoFamiliar.DataBind()
    ...
    Protected Sub ibt_Modificar_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Else
            ...
            Dim inGF As String = lbl_Grupo_Fam.Text
            ...
            For Each row As DataRow In dt.Rows
                ...
                If inGF = row("IdGrupoFamiliar_Indice").ToString() Then
            ...
            'Dim idGF As String = Session("idGF_Mod_Mod")
            ...
            '    If idGF = row("IdGrupoFamiliar_Indice").ToString() Then
    ...
    Protected Sub GV_GrupoFamiliar_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GV_GrupoFamiliar.RowCommand
        ...
        Else
            ...
            If e.CommandName = "Editar" Then
                ...
                lbl_Grupo_Fam.Text = idGF.ToString()
                Dim dt As DataTable = Session("datosGF_Mod")
                Dim indice As Integer = 0
                Dim indiceModificar As Integer = -1
                For Each row As DataRow In dt.Rows
    ...
    Protected Sub btn_buscar_Click(sender As Object, e As ImageClickEventArgs) Handles btn_buscar.Click
        ...
        buscaNHC()
        Dim NHC_s As String = txt_asi.Text.ToString()

        txt_fecha_ES.Text = Nothing
        ddl_circuito.SelectedIndex = 0
        ...
        If GV_pnl_SE.Rows.Count <> 0 Then

            Dim val As Boolean = True
            Dim cont As Integer = 0
            Dim fila As Integer = 0
            Dim TS_IdBasales As String = ""

            lbl_error.Text = ""
            For Each row As GridViewRow In GV_pnl_SE.Rows
                If row.RowType = DataControlRowType.DataRow Then
                    cont = cont + 1
                    Dim chkRow As CheckBox = TryCast(row.Cells(5).FindControl("Check_box"), CheckBox)
                    Dim id As Label = TryCast(row.Cells(0).FindControl("IdBasalesTS"), Label)
                    If chkRow.Checked Then
                        If val Then
                            fila = cont
                            TS_IdBasales = id.Text
                            val = False
                        Else
                            lbl_error.Text = "Seleccionar solo un estudio a modificar"
                            For Each row1 As GridViewRow In GV_pnl_SE.Rows
                                If row1.RowType = DataControlRowType.DataRow Then
                                    Dim chkRow1 As CheckBox = TryCast(row1.Cells(5).FindControl("Check_box"), CheckBox)
            ...
            lbl_id_basal_ts.Text = TS_IdBasales.ToString()
            llenaGrupoFamiliar(TS_IdBasales.ToString())
            llenamodificacion(TS_IdBasales.ToString())
        Else
            llenaestudios(NHC_s)
        End If
    ...
    Sub Limpiar()
        ...
        txt_nambientes.Text = ""
        cbl_tipovivienda.SelectedIndex = -1
        cbl_servicios.SelectedIndex = -1
        cbl_tipoconstruccion.SelectedIndex = -1




        txt_ingpersonal.Text = "0.00"
        txt_inghogar.Text = "0.00"

        txt_egvivienda.Text = "0.00"
        txt_egeelectrica.Text = "0.00"
        txt_egagua.Text = "0.00"
        txt_egcable.Text = "0.00"
        txt_egtelefono.Text = "0.00"
        txt_egalimentacion.Text = "0.00"
        txt_egtransporte.Text = "0.00"
        txt_egeducacion.Text = "0.00"
        txt_egbasura.Text = "0.00"
        txt_egotros.Text = "0.00"

        lbl_totalingresos.Text = "0.00"
        lbl_totalegresos.Text = "0.00"
        lbl_deficit_superavit.Text = "0.00"




        cbl_probidentificados.SelectedIndex = -1
    ...
    Protected Sub btnGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        usuario = Session("usuario").ToString()
        Dim datos As String
        Dim idts As String = lbl_id_basal_ts.Text.ToString()
        'Dim existe As Integer
        ...
        Try
            ...
            If txt_fecha_ES.Text <> Nothing Then
                ...
                db.Grabar_cambios_basal_ts(idts, Session("idPaciente").ToString(), txt_asi.Text.ToString(), txt_fecha_ES.Text.ToString(), txt_observaciones.Text.ToString(), ddl_circuito.SelectedValue.ToString(), usuario)

                If idts <> String.Empty Then


                    'Eliminar grupo familiar
                    db.delete_registros_GrupoFamiliar(idts, usuario)
                    'Grabar grupo familiar
                    ...
                    For Each row As DataRow In dt.Rows
                        ...
                        datos2 = db.InsertarGrupoFamiliar(idts, Session("idPaciente").ToString(), row("Nombre").ToString(), TR, row("Edad").ToString(), NE, SL, row("Ingreso").ToString(), CD, estado, Session("usuario").ToString())
                    ...
                    db.delete_registros_personadx(idts, usuario)
                    db.delete_registros_tipovi(idts, usuario)
                    db.delete_registros_tipoconst(idts, usuario)
                    db.delete_registros_servicios(idts, usuario)
                    db.delete_registros_prob(idts, usuario)
                    db.delete_registros_condicionesvi(idts, usuario)


                    For Each li As System.Web.UI.WebControls.ListItem In cbl_qsDx.Items
                        If li.Selected Then
                            db.Grabar_personaDX(idts, li.Value.ToString(), usuario)
                        End If
                    Next

                    Dim idcondiciones_vivienda As String = db.Condiciones_vivienda(idts, txt_nambientes.Text.ToString(), usuario)

                    If idcondiciones_vivienda <> 0 Then

                        For Each li As System.Web.UI.WebControls.ListItem In cbl_tipovivienda.Items
                            If li.Selected Then
                                db.Tipo_vivienda(idts, idcondiciones_vivienda, li.Value.ToString(), usuario)
                            End If
                        Next

                        For Each li As System.Web.UI.WebControls.ListItem In cbl_tipoconstruccion.Items
                            If li.Selected Then
                                db.Tipo_construccion(idts, idcondiciones_vivienda, li.Value.ToString(), usuario)
                            End If
                        Next

                        For Each li As System.Web.UI.WebControls.ListItem In cbl_servicios.Items
                            If li.Selected Then
                                db.Servicios_vivienda(idts, idcondiciones_vivienda, li.Value.ToString(), usuario)
                            End If
                        Next
                    End If
                    ...
                    For Each li As System.Web.UI.WebControls.ListItem In cbl_probidentificados.Items
                        If li.Selected Then
                            db.Prob_identificados(idts, li.Value.ToString(), usuario)
                        End If
                    Next

                    datos = txt_ingpersonal.Text.ToString() + "|" + txt_inghogar.Text.ToString() + "|" + txt_egvivienda.Text.ToString() + "|" + txt_egeelectrica.Text.ToString() + "|"
                    ...
                    db.Insert_Ingresos_cambios(datos, idts, usuario)
                    Response.Redirect("~/Mod_BasalTS.aspx", False)
                End If
            Else
                lbl_error.Text = "Ingresar Fecha de Estudio Clínico"
            End If
            ' End If
    ...
    Protected Sub btn_abrir_Click(sender As Object, e As EventArgs) Handles btn_abrir.Click
    ...
    Private Sub llenamodificacion(ByVal idts As String)
        db.Cn1 = cn1
        usuario = Session("usuario").ToString()
        ...
        If idts <> "" Then
            ...
            Dim tb_datosbasal_ts As DataTable = db.Search_BasalTS(idts, usuario)
            lbl_id_basal_ts.Text = idts
            txt_fecha_ES.Text = tb_datosbasal_ts.Rows(0).Item(1)
            txt_observaciones.Text = tb_datosbasal_ts.Rows(0).Item(2)
            ddl_circuito.SelectedValue = tb_datosbasal_ts.Rows(0).Item(3).ToString()



            Dim tb_sabe_diag As DataTable = db.Sabe_diag(idts, usuario)
            If tb_sabe_diag IsNot Nothing Then
                cbl_qsDx.Enabled = True
                Dim reader_sabe_diag As DataTableReader = tb_sabe_diag.CreateDataReader()
                While reader_sabe_diag.Read()
                    Dim cbl As System.Web.UI.WebControls.ListItem = cbl_qsDx.Items.FindByValue(reader_sabe_diag("PersonasDx").ToString())
            ...
            Dim tb_condicionesVi As DataTable = db.CondVivi(idts, usuario)
            If tb_condicionesVi.Rows.Count <> 0 Then
                If tb_condicionesVi.Rows(0).Item(0) <> 0 Then
                    txt_nambientes.Text = tb_condicionesVi.Rows(0).Item(1).ToString()
                End If
            End If

            Dim tb_tipo_vivienda As DataTable = db.TipoVivi(idts, usuario)
                If tb_tipo_vivienda IsNot Nothing Then
                    cbl_tipovivienda.Enabled = True
                    Dim reader_tipo_vi As DataTableReader = tb_tipo_vivienda.CreateDataReader()
                    While reader_tipo_vi.Read()
                        Dim cbl As System.Web.UI.WebControls.ListItem = cbl_tipovivienda.Items.FindByValue(reader_tipo_vi("TipoVivienda").ToString())
                        If cbl IsNot Nothing Then
                            cbl.Selected = True
                        End If
                    End While
                ...
                Dim tb_servicios As DataTable = db.Servicios_vi(idts, usuario)
                If tb_servicios IsNot Nothing Then
                    cbl_servicios.Enabled = True
                    Dim reader_servicios_vi As DataTableReader = tb_servicios.CreateDataReader()
                    While reader_servicios_vi.Read()
                        Dim cbl As System.Web.UI.WebControls.ListItem = cbl_servicios.Items.FindByValue(reader_servicios_vi("Servicios").ToString())
                        If cbl IsNot Nothing Then
                            cbl.Selected = True
                        End If
                    End While
                ...
                Dim tb_construcion As DataTable = db.Tipo_constru(idts, usuario)
                If tb_construcion IsNot Nothing Then
                    cbl_tipoconstruccion.Enabled = True
                    Dim reader_construccion As DataTableReader = tb_construcion.CreateDataReader()
                    While reader_construccion.Read()
                        Dim cbl As System.Web.UI.WebControls.ListItem = cbl_tipoconstruccion.Items.FindByValue(reader_construccion("TipoConstruccion").ToString())
                        If cbl IsNot Nothing Then
                            cbl.Selected = True
                        End If
                    End While
                ...
                Dim tb_prob As DataTable = db.ProbIden(idts, usuario)
                If tb_prob IsNot Nothing Then
                    cbl_probidentificados.Enabled = True
                    Dim reader_prob As DataTableReader = tb_prob.CreateDataReader()
                    While reader_prob.Read()
                        Dim cbl As System.Web.UI.WebControls.ListItem = cbl_probidentificados.Items.FindByValue(reader_prob("ProbIdentificados").ToString())
                        If cbl IsNot Nothing Then
                            cbl.Selected = True
                        End If
                    End While
                ...
                Dim tb_ingresos As DataTable = db.Search_Ingresos(idts, usuario)
                Try
                    If tb_ingresos.Rows(0).Item(0) <> 0 Then
                        txt_ingpersonal.Text = tb_ingresos.Rows(0).Item(1).ToString()
                        txt_inghogar.Text = tb_ingresos.Rows(0).Item(2).ToString()
                        txt_egvivienda.Text = tb_ingresos.Rows(0).Item(3).ToString()
                        txt_egeelectrica.Text = tb_ingresos.Rows(0).Item(4).ToString()
                        txt_egagua.Text = tb_ingresos.Rows(0).Item(5).ToString()
                        txt_egcable.Text = tb_ingresos.Rows(0).Item(6).ToString()
                        txt_egtelefono.Text = tb_ingresos.Rows(0).Item(7).ToString()
                        txt_egalimentacion.Text = tb_ingresos.Rows(0).Item(8).ToString()
                        txt_egtransporte.Text = tb_ingresos.Rows(0).Item(9).ToString()
                        txt_egeducacion.Text = tb_ingresos.Rows(0).Item(10).ToString()
                        txt_egbasura.Text = tb_ingresos.Rows(0).Item(11).ToString()
                        txt_egotros.Text = tb_ingresos.Rows(0).Item(12).ToString()
                        lbl_totalingresos.Text = tb_ingresos.Rows(0).Item(13).ToString()
                        lbl_totalegresos.Text = tb_ingresos.Rows(0).Item(14).ToString()
                        lbl_deficit_superavit.Text = tb_ingresos.Rows(0).Item(15).ToString()
                    End If
